...
def create_file_record(
    ...
    for i, (chunk_text, emb) in enumerate(zip(chunks, embeddings), start=1):
        db.add(
            ChunkRecord(
                file_id=record.id, chunk_index=i, text=chunk_text, embedding=emb
            )
        )
...
def search_files(
    ...
    return [
        (item["file"], round(item["score"], 4))
        for item in sorted_results[offset : offset + limit]
    ]
...
def search_files_semantic(
    ...
    raw = (
        db.query(
            FileRecord,
            sa_cast(ChunkRecord.embedding.op("<=>")(query_vec), Float).label(
                "distance"
            ),
        )
        .join(ChunkRecord, ChunkRecord.file_id == FileRecord.id)
...
def search_chunks(db: Session, user_id: int, query: str, limit: int = 10):
    if not query.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Query cannot be empty."
        )
    ...
    return [{"text": chunk.text, "filename": filename} for chunk, filename in results]
